...
- **Test Statistic (z-test, known variance)**:
  \[
  z = \frac{\bar{x} - \mu_0}{\sigma / \sqrt{n}} = \frac{3 - 0}{10 / \sqrt{50}} = \frac{3}{1.414} \approx 2.12
  \]
...
- **Type II Error**:
  ...
  - Calculating \(\beta\) requires the true \(\mu\). Suppose the true \(\mu = 5\):
    - Under \(H_1\), the test statistic’s distribution is centered at:
      \[
      z = \frac{5 - 0}{10 / \sqrt{50}} \approx 3.54
      \]
    - The critical value for \(\alpha = 0.05\) (right-tailed) is \(z = 1.645\).
    - \(\beta = P(Z < 1.645 | \mu = 5)\), which requires computing the z-score under the alternative distribution:
      \[
      z = \frac{1.645 \cdot 1.414 - 5}{1.414} \approx -1.92
      \]
      \[
      \beta = P(Z < -1.92) \approx 0.027
      \]
    - Power: \(1 - \beta = 1 - 0.027 = 0.973\).
