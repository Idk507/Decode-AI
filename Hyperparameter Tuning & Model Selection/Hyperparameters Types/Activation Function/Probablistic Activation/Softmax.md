...
| ----------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Range**               | $(0, 1)$, with $\displaystyle \sum_{i=1}^n \text{Softmax}(x_i) = 1$.                                                                                                                                                                                                                     |
| **Derivative**          | Captured by the **Jacobian matrix**:<br><br> $\displaystyle \frac{\partial \text{Softmax}(x_i)}{\partial x_j} = \begin{cases} \text{Softmax}(x_i)(1 - \text{Softmax}(x_i)) & \text{if } i = j \\[10pt] -\text{Softmax}(x_i) \cdot \text{Softmax}(x_j) & \text{if } i \neq j \end{cases}$ |
| **Behavior**            | - Large positive $x_i$: output near 1.<br> - Large negative $x_i$: output near 0.<br> - Equal inputs: uniform distribution.                                                                                                                                                              |
| **Numerical Stability** | To avoid overflow, subtract $\max(\mathbf{x})$:<br><br> $\displaystyle \text{Softmax}(x_i) = \frac{e^{x_i - \max(\mathbf{x})}}{\sum_j e^{x_j - \max(\mathbf{x})}}$                                                                                                                       |
